...
export default function MarkPage() {
  ...
  const orchards = useLiveQuery(() =>
    db.orchards.orderBy("name").filter((o) => o.active).toArray()
  );
  ...
  const activeShiftCounts = useLiveQuery(
    () =>
      activeShift
        ? db.counts.where("shiftId").equals(activeShift.id).toArray()
        : [],
    [activeShift]
  );
  ...
  const pickerCards = useMemo(() => {
    ...
    return activeShiftCounts
      ...
      .filter((item) => {
        return (
          item.pickerName.toLowerCase().includes(term) ||
          item.pickerNickname.toLowerCase().includes(term)
        );
      })
      .sort((a, b) => {
        if (sortBy === "boxes") {
  ...
  const shiftTotalBoxes = useMemo(
    () => activeShiftCounts?.reduce((sum, c) => sum + c.boxes, 0) || 0,
    [activeShiftCounts]
  );
  ...
  const updateCount = async (countId: string, delta: number) => {
    ...
    try {
      ...
      if (navigator.vibrate) {
        if (delta > 0) navigator.vibrate(50);
  ...
  if (!workday) {
    return (
      <div className="p-10 text-center font-bold text-lg text-red-700">
        Dia não encontrado.
      </div>
    );
  }
      ...
      <div className="flex flex-col min-h-screen bg-gray-100">
        {isEasyMode ? (
          <div className="bg-blue-800 text-white p-3 flex justify-between items-center shadow-lg shrink-0">
            <Link
              href="/"
              className="flex items-center gap-2 p-2 bg-blue-900 rounded-lg border border-blue-600"
            >
            ...
            <Link
              href={`/day/${workdayId}/summary`}
              className="flex flex-col items-center bg-blue-900 p-2 rounded-lg border border-blue-600"
            >
        ...
        <main className="max-w-md mx-auto p-6 space-y-4 w-full">
          <div className="bg-white rounded-xl border-4 border-gray-900 shadow-lg p-5">
            <p className="font-black text-xl text-gray-900 mb-2">
              Nenhum local criado ainda
            </p>
            <p className="text-sm font-bold text-gray-600">
              Para começar, crie o <span className="text-gray-900">1º Local</span>{" "}
              (Pomar + Preço).
            </p>
          ...
          <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden border-4 border-gray-900">
              <div className="bg-blue-900 p-4 text-white flex justify-between items-center">
                <h3 className="font-black text-xl uppercase">Criar Local</h3>
                <button
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-4 border-2 border-gray-900 rounded-lg bg-white text-lg font-bold text-black h-16"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-4 border-2 border-gray-900 rounded-lg bg-white text-2xl font-black text-black"
                    inputMode="decimal"
                  />
                </div>
                ...
                <div className="pt-2 flex gap-3">
                  ...
                  <Button size="lg" onClick={handleCreateShift} className="flex-1">
                    CRIAR
                  </Button>
                </div>
              </div>
    ...
    <div className="flex flex-col h-screen bg-gray-100">
      ...
      {isClosed ? (
        ...
        <div
          className={`${
            isEasyMode
              ? "bg-white text-gray-900 border-b-4 border-gray-300 py-4"
              : "bg-blue-900 text-white py-3 border-b-4 border-blue-950"
          } px-3 flex justify-between items-center shadow-lg z-30`}
        >
          <div>
            <div className="flex items-center gap-2">
              ...
              >
                {activeShift.orchardNameSnapshot}
              </span>
            </div>
            <div
              className={`${
                isEasyMode ? "text-gray-600" : "text-blue-200"
          ...
          <Button
            size="sm"
            variant="outline"
            className={`h-12 text-xs border-2 ${
              isEasyMode
      ...
      {!isEasyMode && (
        <div className="px-3 py-3 bg-white border-b-2 border-gray-300 shadow-sm flex gap-2 sticky top-[120px] z-40">
          ...
          >
            {viewMode === "grid" ? (
              <ListIcon className="w-6 h-6 text-gray-900" />
            ) : (
      ...
      {/* PICKERS GRID/LIST */}
      <div
        className={`flex-1 overflow-y-auto p-3 pb-56 space-y-4 ${
          viewMode === "grid" && !isEasyMode
      ...
      >
        ...
        {pickerCards.map((card) => (
          ...
          >
            {/* Header */}
            <div
              className={`p-3 bg-gray-50 border-b-2 border-gray-900 flex justify-between items-center rounded-t-[10px] shrink-0 ${
                isEasyMode ? "py-4" : ""
              }`}
            >
              <div className="overflow-hidden">
                <h3
                  className={`font-black text-gray-900 truncate leading-tight ${
                    isEasyMode ? "text-2xl" : "text-xl"
                  }`}
                >
                  {card.pickerName}
                </h3>
                {card.pickerNickname && (
                  <p className="text-gray-600 font-bold text-sm truncate">
                    "{card.pickerNickname}"
                  </p>
                )}
              </div>
      ...
      <div
        className={`fixed bottom-0 left-0 right-0 ${
          isEasyMode
            ? "bg-white border-t-4 border-blue-900 pb-safe"
            : "bg-gray-900 border-t-4 border-gray-700 pb-safe bottom-20"
        } p-3 shadow-2xl z-40`}
      >
        <div className="max-w-md mx-auto flex justify-between items-center">
          <div>
            <p
              className={`${
                isEasyMode ? "text-gray-500" : "text-gray-400"
            ...
            <p
              className={`${
                isEasyMode ? "text-gray-900" : "text-white"
          ...
          <div className="text-right">
            ...
            <p
              className={`text-3xl font-black ${
                isEasyMode ? "text-green-700" : "text-green-400"
              } leading-none`}
            >
              {formatCurrency(shiftTotalValue)}
            </p>
          </div>
        </div>
      </div>
      ...
      {showShiftModal && (
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden border-4 border-gray-900">
            ...
            <div className="p-6 space-y-6">
              <div className="bg-yellow-100 text-yellow-900 font-bold text-sm p-4 rounded border-2 border-yellow-500">
                Isso cria um <strong>novo local de contagem</strong>. As caixas do
                local anterior ficam salvas.
              </div>
