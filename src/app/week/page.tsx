...
/** ✅ Tipos (evita implicit any no build do Vercel) */
...
export default function WeekPage() {
  ...
  const closedWorkdays = useLiveQuery(
    () =>
      db.workdays
        ...
        .filter((w: any) => w?.status === "closed" || !!w?.closedAt)
        .toArray(),
    [startStr, endStr]
  );

  const sortedClosedWorkdays = useMemo(
    () => (closedWorkdays || []).sort((a: any, b: any) => a.date.localeCompare(b.date)),
    [closedWorkdays]
  );

  const closedWorkdayIds = useMemo(
    () => sortedClosedWorkdays.map((w: any) => w.id),
    [sortedClosedWorkdays]
  );
  ...
  const liveData: WeekSummaryData = useMemo(() => {
    if (!sortedClosedWorkdays || !counts || !shifts || !pickers) {
      return {
        ...
        totalWeekBoxes: 0,
        totalWeekValue: 0,
        closedWorkdaysCount: 0,
      };
    }

    // A. Daily Summaries
    const dailySummaries: DailySummary[] = sortedClosedWorkdays.map((day: any) => {
      const dayCounts = counts.filter((c: any) => c.workdayId === day.id);

      // Agrupa por pomar (se mudou de pomar no mesmo dia)
      const orchardMap = new Map<string, OrchardMini>();

      dayCounts.forEach((c: any) => {
        if (!c.boxes || c.boxes <= 0) return;
        const shift = shifts.find((s: any) => s.id === c.shiftId);
        if (!shift) return;

        const key = shift.orchardNameSnapshot || "Sem Pomar";
        const cur = orchardMap.get(key) || {
          orchardName: key,
          totalBoxes: 0,
          totalValue: 0,
        };
      ...
      const orchards = Array.from(orchardMap.values()).sort(
    ...
    counts.forEach((c: any) => {
      const shift = shifts.find((s: any) => s.id === c.shiftId);
      const workday = sortedClosedWorkdays.find((w: any) => w.id === c.workdayId);
      if (!shift || !workday || c.boxes <= 0) return;

      const p = pickers.find((pk: any) => pk.id === c.pickerId);
      ...
      if (!current.dailyBreakdown[workday.date]) {
        current.dailyBreakdown[workday.date] = {
          date: workday.date,
          weekDay: format(new Date(workday.date + "T00:00:00"), "EEEE", {
            locale: ptBR,
          }),
          boxes: 0,
          value: 0,
    ...
    const pickersAggregated = Array.from(map.values()).sort(
      (a, b) => b.totalBoxes - a.totalBoxes
    );

    const totalWeekBoxes = pickersAggregated.reduce(
      (acc, curr) => acc + curr.totalBoxes,
      0
    );

    const totalWeekValue = pickersAggregated.reduce(
  ...
  const displayData: WeekSummaryData = useMemo(() => {
    if (isWeekClosed && (weekRecord as any)?.snapshot) {
      return (weekRecord as any).snapshot as WeekSummaryData;
    }
    return liveData;
  ...
  const exportCSV = () => {
    ...
    displayData.pickersAggregated.forEach((p) => {
      ...
      dates.forEach((date) => {
        ...
        csvContent += `${format(new Date(d.date + "T00:00:00"), "dd/MM/yyyy")};${
          d.weekDay
        };${p.name};${d.boxes};${d.value.toFixed(2).replace(".", ",")}\n`;
      });
    });

    csvContent += `\n;;TOTAL GERAL;${displayData.totalWeekBoxes};${displayData.totalWeekValue
      .toFixed(2)
      .replace(".", ",")}\n`;
    ...
    FileSaver.saveAs(
      blob,
      `resumo_semana_${isWeekClosed ? "FECHADA_" : ""}${startStr}.csv`
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <main className="max-w-md mx-auto p-4 space-y-6">
        ...
        <div
          className={`p-3 rounded-xl shadow-sm border-4 ${
            isWeekClosed ? "bg-red-50 border-red-200" : "bg-white border-gray-900"
          }`}
        >
          <div className="flex justify-between items-center mb-2">
            <button
              onClick={() => setCurrentDate((d) => subWeeks(d, 1))}
              className="p-2 hover:bg-gray-100 rounded text-blue-800"
            >
              <ChevronLeft className="w-8 h-8" />
            </button>
            <div className="text-center">
              {isWeekClosed && (
                <span className="text-[10px] bg-red-600 text-white px-2 py-0.5 rounded font-black mb-1 inline-block">
                  SEMANA FECHADA
                </span>
              )}
              <p
                className={`text-xs font-black uppercase tracking-wider ${
                  isWeekClosed ? "text-red-800" : "text-gray-500"
                }`}
              >
                SEMANA (SEG - DOM)
              </p>
          ...
          <div className="flex justify-between items-center text-xs px-2 pt-2 border-t border-gray-100">
            ...
            <button
              onClick={exportCSV}
              className="text-blue-800 font-bold flex items-center gap-1 uppercase"
            >
        ...
        {!isWeekClosed && openWorkdaysCount && openWorkdaysCount > 0 ? (
          <div className="bg-yellow-50 border-2 border-yellow-400 rounded-xl p-3 flex gap-3 items-center">
            ...
            <div className="text-sm font-bold text-yellow-900 flex-1">
              Há {openWorkdaysCount} dia(s) em aberto. Feche os dias para fechar a
              semana.
            </div>
          </div>
        ) : null}
          ...
          {isWeekClosed ? (
            <div className="bg-white p-4 rounded-xl border-2 border-red-100 shadow-sm text-center space-y-3">
              ...
              <p className="text-xs text-gray-500">
                Fechado em{" "}
                {(weekRecord as any)?.closedAt
                  ? format(new Date((weekRecord as any).closedAt), "dd/MM HH:mm")
                  : "-"}
              </p>
              <Button
                variant="secondary"
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-3">
            {displayData.dailySummaries.map((day: DailySummary) => (
                ...
                {isWeekClosed ? (
                  <div className="p-3 flex justify-between items-center cursor-default">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-black text-gray-700 text-lg capitalize">
                          {day.weekDay}
                        </span>
                      ...
                      <div className="flex gap-3 text-xs mt-1">
                        <span className="bg-gray-200 text-gray-700 px-2 rounded font-bold">
                          Total: {day.totalBoxes} cx
                        </span>
                      </div>

                      {day.orchards?.length ? (
                        <div className="flex flex-wrap gap-2 text-xs mt-2">
                          {day.orchards.map((o: OrchardMini) => (
                            <span
                              key={o.orchardName}
                  ...
                  <Link href={`/day/${day.id}/summary`} className="block active:bg-gray-50">
                    <div className="p-3 flex justify-between items-center">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-black text-gray-900 text-lg capitalize">
                            {day.weekDay}
                          </span>
                          <span className="text-sm text-gray-500 font-bold">
                            {day.dateFormatted}
                        ...
                        <div className="flex gap-3 text-xs mt-1">
                          <span className="bg-blue-100 text-blue-900 px-2 rounded font-bold">
                            Total: {day.totalBoxes} cx
                          </span>
                        </div>
        ...
        <div className="space-y-2 pt-4">
          ...
          <div className="space-y-3">
            {displayData.pickersAggregated.map((picker) => {
              ...
              const days = (Object.values(picker.dailyBreakdown) as DailyBreakdown[]).sort(
              ...
              return (
                <div
                  key={picker.pickerId}
                  className="bg-white rounded-xl border-2 border-gray-900 shadow-sm overflow-hidden transition-all"
                >
                  {/* Accordion Header */}
                  <div
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <p className="font-black text-2xl text-gray-900">
                          {picker.totalBoxes}{" "}
                          <span className="text-xs font-bold text-gray-400">cx</span>
                        </p>
                      </div>
                      {isExpanded ? (
                    ...
                    <div className="border-t-2 border-gray-200 bg-white animate-in slide-in-from-top-2">
                      ...
                      <div className="bg-green-700 text-white p-3 flex justify-between items-center">
                        ...
                        <span className="text-xl font-black">
                          {formatCurrency(picker.totalValue)}
                        </span>
                      </div>

                      {/* Detailed List by Day */}
                      <div className="p-3 space-y-2">
                        <p className="text-[10px] text-gray-400 uppercase font-black pl-1">
                          Produção Diária
                        </p>
                        {days.map((day, idx) => (
                          <div
            ...
            <div className="max-w-md mx-auto flex justify-between items-center">
              <div>
                ...
                <p className="text-2xl font-black">
                  {displayData.totalWeekBoxes}{" "}
                  <span className="text-sm font-bold text-gray-500">cx</span>
                </p>
              </div>
              <div className="text-right">
                <p className="text-[10px] text-gray-400 font-black uppercase tracking-wider">
                ...
                <p className="text-2xl font-black text-green-400">
                  {formatCurrency(displayData.totalWeekValue)}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="p-4 overflow-y-auto grow">
                ...
                <div className="space-y-2">
                  <p className="text-xs font-black text-gray-500 uppercase">
                    Resumo ({displayData.pickersAggregated.length} colhedores)
                  </p>
                  ...
                  {displayData.pickersAggregated.length > 5 && (
                    <p className="text-xs text-center text-gray-400 italic">
                      ...e mais {displayData.pickersAggregated.length - 5}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm border-4 border-red-500">
              <div className="bg-red-100 p-4 text-red-900">
                ...
                <p className="text-sm font-bold">
                  Cuidado: O relatório congelado será descartado. Os valores serão
                  recalculados com base nos dias atuais.
                </p>
              </div>
              <div className="p-4 flex gap-3">
                <Button
                  variant="outline"
                  onClick={() => setShowReopenWeekModal(false)}
                  className="flex-1"
                >
                  Cancelar
                </Button>
